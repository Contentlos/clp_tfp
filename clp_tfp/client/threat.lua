-- clp_tfp · threat/ (Client) — Phase 3: Downed-Zustand + Loot-Drop bei endgültigem Tod

local ESX = exports['es_extended']:getSharedObject()
TFP = TFP or {}

local cfg = Config.Threat.downed or {}
local downedPlayers = {} -- [serverId] = true (Revive-Ziele)

local function realDeath()
    TFP.downed = false
    TriggerServerEvent('clp_tfp:setDowned', false)
    if TFP.SetDowned then TFP.SetDowned(true, 0, 'dead') end
    if Config.Threat.dropInventoryOnDeath then
        TriggerServerEvent('clp_tfp:died')
        Wait(600) -- Server legt Beutel am Sterbeort ab, bevor wir wegteleportieren
    end
    Wait(900) -- kurzer "GESTORBEN"-Moment
...
local function enterDowned()
    ...
    AnimpostfxPlay('DeathFailOut') -- klassischer entsättigter "Wasted"-Look
    ...
    CreateThread(function()
        while TFP.downed and GetGameTimer() < deadline do
            ...
            DisableControlAction(0, 23, true) -- Einsteigen
            local left = math.max(0, math.ceil((deadline - GetGameTimer()) / 1000))
            if left ~= lastShown then
                lastShown = left
                if TFP.SetDowned then TFP.SetDowned(true, left, 'downed') end
            end
            if IsControlJustPressed(0, 73) then break end -- X = aufgeben
        end
...
RegisterNetEvent('clp_tfp:revived', function()
    ...
    if TFP.ResetSurvival then TFP.ResetSurvival() end  -- Verletzungen zurücksetzen, sonst sofort wieder downed
    lib.notify({ title = 'Gerettet', description = 'Du wurdest wiederbelebt.', type = 'success' })
end)

RegisterNetEvent('clp_tfp:playerDowned', function(serverId, state)
    downedPlayers[serverId] = state or nil
end)

-- Revive-Option an Mitspielern (nur wenn sie "downed" sind)
CreateThread(function()
        ...
        exports.ox_target:addGlobalPlayer({
            {
                ...
                canInteract = function(entity)
                    ...
                    return downedPlayers[sid] == true
                end,
                onSelect = function(data)
                    local sid = GetPlayerServerId(NetworkGetPlayerIndexFromPed(data.entity))
                    if (exports.ox_inventory:Search('count', cfg.reviveItem) or 0) < 1 then
                        lib.notify({ description = 'Du brauchst dafür ein Erste-Hilfe-Set.', type = 'error' }); return
                    end
    ...
    if not ok and TFP_Warn then TFP_Warn('Revive-Target (addGlobalPlayer) nicht verfügbar: ' .. tostring(err)) end
end)
