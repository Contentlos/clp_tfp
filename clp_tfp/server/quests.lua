-- clp_tfp · quests/ (Server) — Survival-Aufgaben: Fortschritt verfolgen + belohnen
-- Fortschritt session-basiert (im Speicher). Wiederholbar (Config.QuestRepeat).

local progress = {}  -- [src] = { [questId] = count }

-- Von anderen Server-Skripten aufgerufen: TFP_QuestProgress(src, type, key, amount)
function TFP_QuestProgress(src, qtype, key, amount)
