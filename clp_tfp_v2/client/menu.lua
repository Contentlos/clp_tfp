-- clp_tfp · client/menu — Hauptmenü (F5 / /survival / Radial) mit Untermenüs pro Bereich
...
**Müdigkeit:** %d%%   **Nässe:** %d%%
**Blutung:** %d%%   **Bruch:** %s
**Wunden:** %d   **Krankheit:** %s]]):format(
            hp, pct(s.blood), pct(s.hunger), pct(s.thirst), pct(s.stamina),
            pct(s.temperature), pct(s.fatigue), pct(s.wetness), pct(s.bleeding), frac, nWounds, disease),
...
local function openSurvival()
    ...
    local nWounds = #((TFP.State and TFP.State.wounds) or {})
    o[#o + 1] = { title = 'Wundversorgung', icon = 'fa-solid fa-kit-medical', arrow = true,
        description = (nWounds > 0) and ('%d offene Wunde(n) — gezielt behandeln'):format(nWounds) or 'Wunden ansehen & behandeln',
        onSelect = function() if TFP.OpenWoundMenu then TFP.OpenWoundMenu() end end }
    o[#o + 1] = { title = 'Kleidung', icon = 'fa-solid fa-shirt', arrow = true,
        description = 'Schutzkleidung an-/ablegen (Wärme · Nässe · Panzer · Tarnung)',
        onSelect = function() if TFP.OpenClothing then TFP.OpenClothing() end end }
...
local function openActivities()
    ...
    o[#o + 1] = { title = 'Tagebuch', icon = 'fa-solid fa-book', description = 'Gefundene Lore-Seiten lesen',
        onSelect = function() if TFP.OpenJournal then TFP.OpenJournal() end end }
...
local function openMenu()
    lib.registerContext({ id = 'tfp_menu', title = '🌴 Survival', options = {
        ...
        { title = 'Aktivitäten',       icon = 'fa-solid fa-compass',     description = 'Aufgaben · Tagebuch · Angeln',         arrow = true, onSelect = openActivities },
    ...
    local ok = pcall(function()
        lib.registerRadial({
            ...
            items = {
                ...
                { label = 'Wunden', icon = 'kit-medical', onSelect = function() if TFP.OpenWoundMenu then TFP.OpenWoundMenu() end end },
                { label = 'Kleidung', icon = 'shirt', onSelect = function() if TFP.OpenClothing then TFP.OpenClothing() end end },
