-- clp_tfp · server/tribe — Stämme, Ränge, geteilter Zugriff, Marker. Liefert TFP_GetTribeId/AreSameTribe.
local ESX = exports['es_extended']:getSharedObject()

local membersByIdent = {}
local tribes = {}
local invites = {}

CreateThread(function()
    MySQL.query.await([[CREATE TABLE IF NOT EXISTS `tfp_tribes` ( `id` INT NOT NULL AUTO_INCREMENT, `name` VARCHAR(48) NOT NULL, `owner` VARCHAR(64) NOT NULL, PRIMARY KEY (`id`) ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;]])
    MySQL.query.await([[CREATE TABLE IF NOT EXISTS `tfp_tribe_members` ( `tribe_id` INT NOT NULL, `identifier` VARCHAR(64) NOT NULL, `rank` VARCHAR(16) NOT NULL DEFAULT 'member', `name` VARCHAR(48) NOT NULL DEFAULT '', PRIMARY KEY (`identifier`) ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;]])
    for _, t in ipairs(MySQL.query.await('SELECT * FROM tfp_tribes') or {}) do tribes[t.id] = { id = t.id, name = t.name, owner = t.owner, members = {} } end
...
function TFP_GetTribeId(identifier) local m = membersByIdent[identifier]; return m and m.tribeId or nil end

local function tribeData(identifier)
    local m = membersByIdent[identifier]; if not m then return nil end
...
RegisterNetEvent('clp_tfp:tribeCreate', function(name)
    ...
    if membersByIdent[ident] then TriggerClientEvent('ox_lib:notify', src, { description = 'Du bist schon in einem Stamm.', type = 'error' }); return end
    name = (name or ''):sub(1, 48)
    if #name < 3 then TriggerClientEvent('ox_lib:notify', src, { description = 'Name zu kurz.', type = 'error' }); return end
    ...
    MySQL.insert('INSERT INTO tfp_tribe_members (tribe_id, identifier, rank, name) VALUES (?,?,?,?)', { id, ident, 'leader', GetPlayerName(src) })
...
RegisterNetEvent('clp_tfp:tribeInvite', function(targetId)
    ...
    if not m or (m.rank ~= 'leader' and m.rank ~= 'officer') then TriggerClientEvent('ox_lib:notify', src, { description = 'Keine Berechtigung.', type = 'error' }); return end
    local t = tribes[m.tribeId]; if not t then return end
    local count = 0; for _ in pairs(t.members) do count = count + 1 end
    if count >= Config.Tribe.maxMembers then TriggerClientEvent('ox_lib:notify', src, { description = 'Stamm ist voll.', type = 'error' }); return end
    local tx = ESX.GetPlayerFromId(targetId); if not tx then return end
    if membersByIdent[tx.getIdentifier()] then TriggerClientEvent('ox_lib:notify', src, { description = 'Spieler ist schon in einem Stamm.', type = 'error' }); return end
...
RegisterNetEvent('clp_tfp:tribeAccept', function()
    ...
    if not tribeId or not tribes[tribeId] then TriggerClientEvent('ox_lib:notify', src, { description = 'Keine offene Einladung.', type = 'error' }); return end
    invites[ident] = nil
    tribes[tribeId].members[ident] = 'member'
    membersByIdent[ident] = { tribeId = tribeId, rank = 'member', name = GetPlayerName(src) }
    MySQL.insert('INSERT INTO tfp_tribe_members (tribe_id, identifier, rank, name) VALUES (?,?,?,?)', { tribeId, ident, 'member', GetPlayerName(src) })
...
RegisterNetEvent('clp_tfp:tribeLeave', function()
    ...
    if t then
        t.members[ident] = nil
        if t.owner == ident then
            for mid in pairs(t.members) do membersByIdent[mid] = nil; MySQL.update('DELETE FROM tfp_tribe_members WHERE identifier = ?', { mid }) end
            tribes[t.id] = nil; MySQL.update('DELETE FROM tfp_tribes WHERE id = ?', { t.id })
...
local function srcByIdent(ident)
    for _, pid in ipairs(GetPlayers()) do local px = ESX.GetPlayerFromId(tonumber(pid)); if px and px.getIdentifier() == ident then return tonumber(pid) end end
end
...
RegisterNetEvent('clp_tfp:tribePromote', function(targetIdent)
    local src = source; local xPlayer = ESX.GetPlayerFromId(src); if not xPlayer then return end
    local m = membersByIdent[xPlayer.getIdentifier()]
    if not m or m.rank ~= 'leader' then TriggerClientEvent('ox_lib:notify', src, { description = 'Nur der Anführer kann befördern.', type = 'error' }); return end
    ...
    local ts = srcByIdent(targetIdent); if ts then TriggerClientEvent('ox_lib:notify', ts, { title = 'Stamm', description = 'Du bist jetzt Offizier.', type = 'inform' }) end
end)
RegisterNetEvent('clp_tfp:tribeDemote', function(targetIdent)
    local src = source; local xPlayer = ESX.GetPlayerFromId(src); if not xPlayer then return end
    local m = membersByIdent[xPlayer.getIdentifier()]
    if not m or m.rank ~= 'leader' then TriggerClientEvent('ox_lib:notify', src, { description = 'Nur der Anführer kann herabstufen.', type = 'error' }); return end
    ...
    local ts = srcByIdent(targetIdent); if ts then TriggerClientEvent('ox_lib:notify', ts, { title = 'Stamm', description = 'Du bist jetzt einfaches Mitglied.', type = 'inform' }) end
end)
RegisterNetEvent('clp_tfp:tribeKick', function(targetIdent)
    ...
    if targetIdent == meIdent or targetIdent == t.owner then TriggerClientEvent('ox_lib:notify', src, { description = 'Das geht nicht.', type = 'error' }); return end
    if not ((m.rank == 'leader') or (m.rank == 'officer' and targetRank == 'member')) then TriggerClientEvent('ox_lib:notify', src, { description = 'Keine Berechtigung.', type = 'error' }); return end
    t.members[targetIdent] = nil; membersByIdent[targetIdent] = nil
    ...
    if ts then TriggerClientEvent('ox_lib:notify', ts, { title = 'Stamm', description = 'Du wurdest aus dem Stamm entfernt.', type = 'error' }); TriggerClientEvent('clp_tfp:tribeKicked', ts) end
end)
...
lib.callback.register('clp_tfp:getTribeStash', function(source)
    ...
    exports.ox_inventory:RegisterStash(stashId, ('Stamm: %s'):format(t.name), Config.Tribe.stashSlots or 50, Config.Tribe.stashWeight or 500000)
    return stashId
end)
